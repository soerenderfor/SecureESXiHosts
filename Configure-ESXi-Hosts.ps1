...
foreach ($esxiHost in $esxiHosts) {
    ...
    if ($srv) {
        ...
        # The ESXi host must not suppress warnings that the local or remote shell sessions are enabled - ESXI-80-000222
        $setting = Get-AdvancedSetting -Entity $obj -Name UserVars.SuppressShellWarning
        if ($setting.Value -ne 0) {
            [Void](Set-AdvancedSetting -AdvancedSetting $setting -Value 0 -Confirm:$false)
            Write-LogMessage -Message "$($esxiHost.VMHost): Configuring the ESXi host must not suppress warnings that the local or remote shell sessions are enabled - ESXI-80-000222." -Level UPDATE
        } else {
            Write-LogMessage -Message "$($esxiHost.VMHost): The ESXi host is already configured to not suppress warnings that the local or remote shell sessions are enabled - ESXI-80-000222." -Level 'PASS'
        }
        # The ESXi host must set a timeout to automatically end idle shell sessions after fifteen minutes - V-258739
        $setting = Get-AdvancedSetting -Entity $obj -Name UserVars.ESXiShellInteractiveTimeOut
        if ($setting.Value -ne 900) {
            [Void](Set-AdvancedSetting -AdvancedSetting $setting -Value 900 -Confirm:$false)
            Write-LogMessage -Message "$($esxiHost.VMHost): Configuring the ESXi host to set a timeout to automatically end idle shell sessions after fifteen minutes. V-258739" -Level 'UPDATE'
        } else {
            Write-LogMessage -Message "$($esxiHost.VMHost): The ESXi host is already configured to set a timeout to automatically end idle shell sessions after fifteen minutes. V-258739" -Level 'PASS'
        }

        # The ESXi host must automatically stop shell services after 10 minutes - V-258756
        $setting = Get-AdvancedSetting -Entity $obj -Name UserVars.ESXiShellTimeOut
        if ($setting.Value -ne 600) {
            [Void](Set-AdvancedSetting -AdvancedSetting $setting -Value 600 -Confirm:$false)
            Write-LogMessage -Message "$($esxiHost.VMHost): Configuring the ESXi host to automatically stop shell services after 10 minutes. V-258756" -Level 'UPDATE'
        } else {
            Write-LogMessage -Message "$($esxiHost.VMHost): The ESXi host is already configured to automatically stop shell services after 10 minutes. V-258756" -Level 'PASS'
        }

        # The ESXi host client must be configured with an idle session timeout - V-258731
        $setting = Get-AdvancedSetting -Entity $obj -Name UserVars.HostClientSessionTimeout
        if ($setting.Value -ne 900) {
            [Void](Set-AdvancedSetting -AdvancedSetting $setting -Value 900 -Confirm:$false)
            Write-LogMessage -Message "$($esxiHost.VMHost): Configuring the ESXi host client to set an idle session timeout of 15 minutes. V-258731" -Level 'UPDATE'
        } else {
            Write-LogMessage -Message "$($esxiHost.VMHost): The ESXi host client is already configured with an idle session timeout of 15 minutes. V-258731" -Level 'PASS'
        }

        # The ESXi host must configure a session timeout for the vSphere API. V-258781
        $setting = Get-AdvancedSetting -Entity $obj -Name Config.HostAgent.vmacore.soap.sessionTimeout
        if ($setting.Value -ne 30) {
            [Void](Set-AdvancedSetting -AdvancedSetting $setting -Value 30 -Confirm:$false)
            Write-LogMessage -Message "$($esxiHost.VMHost): Configuring the ESXi host to set a session timeout for the vSphere API to 30 minutes. V-258781" -Level 'UPDATE'
        } else {
            Write-LogMessage -Message "$($esxiHost.VMHost): The ESXi host is already configured with a session timeout for the vSphere API of 30 minutes. V-258781" -Level 'PASS'
        }

        # The ESXi host must set a timeout to automatically end idle DCUI sessions after 10 minutes - V-258757
        $setting = Get-AdvancedSetting -Entity $obj -Name UserVars.DcuiTimeOut
        if ($setting.Value -ne 600) {
            [Void](Set-AdvancedSetting -AdvancedSetting $setting -Value 600 -Confirm:$false)
            Write-LogMessage -Message "$($esxiHost.VMHost): Configuring the ESXi host to set a timeout to automatically end idle DCUI sessions after 10 minutes. V-258757" -Level 'UPDATE'
        } else {
            Write-LogMessage -Message "$($esxiHost.VMHost): The ESXi host is already configured to set a timeout to automatically end idle DCUI sessions after 10 minutes. V-258757" -Level 'PASS'
        }

        # The ESXi host must enable volatile key destruction. V-258780
        $setting = Get-AdvancedSetting -Entity $obj -Name Mem.MemEagerZero
        if ($setting.Value -ne 1) {
            [Void](Set-AdvancedSetting -AdvancedSetting $setting -Value 1 -Confirm:$false)
            Write-LogMessage -Message "$($esxiHost.VMHost): Configuring the ESXi host to enable volatile key destruction. V-258780" -Level 'UPDATE'
        } else {
            Write-LogMessage -Message "$($esxiHost.VMHost): The ESXi host is already configured to enable volatile key destruction. V-258780" -Level 'PASS'
        }
        ...
        $setting = Get-AdvancedSetting -Entity $obj -Name Security.PasswordMaxDays
        if ($setting.Value -ne 9999) {
            [Void](Set-AdvancedSetting -AdvancedSetting $setting -Value 9999 -Confirm:$false)
            Write-LogMessage -Message "$($esxiHost.VMHost): Configuring the ESXi host with an appropriate maximum password age of 9999 days. Better with password complexity settings." -Level 'UPDATE'
        } else {
            Write-LogMessage -Message "$($esxiHost.VMHost): The ESXi host is already configured with an appropriate maximum password age of 9999 days. V-258751" -Level 'PASS'
        }

        # The ESXi host must be configured wth exclusively enabling Transport Layer Security (TLS) 1.2. V-258749
        $setting = Get-AdvancedSetting -Entity $obj -Name UserVars.ESXiVPsDisabledProtocols
        if ($setting.Value -ne 'sslv3,tlsv1,tlsv1.1') {
            [Void](Set-AdvancedSetting -AdvancedSetting $setting -Value 'sslv3,tlsv1,tlsv1.1' -Confirm:$false)
            Write-LogMessage -Message "$($esxiHost.VMHost): Exclusively enabling Transport Layer Security (TLS) 1.2 on host. V-258749" -Level 'UPDATE'
        } else {
            Write-LogMessage -Message "$($esxiHost.VMHost): The ESXi host is already configured with exclusively enabling Transport Layer Security (TLS) 1.2. V-258749" -Level 'PASS'
        }

        # The ESXi Managed Object Browser (MOB) must be deactivated. V-258736
        $setting = Get-AdvancedSetting -Entity $obj -Name Config.HostAgent.plugins.solo.enableMob
        if ($setting.Value -ne $false) {
            [Void](Set-AdvancedSetting -AdvancedSetting $setting -Value $false -Confirm:$false)
            Write-LogMessage -Message "$($esxiHost.VMHost): Disabling the Managed Object Browser (MOB) on host. V-258736" -Level 'UPDATE'
        } else {
            Write-LogMessage -Message "$($esxiHost.VMHost): The Managed Object Browser (MOB) is already disabled on host. V-258736" -Level 'PASS'
        }
        ...
        # The ESXi host must disable virtual hardware management network interfaces. ESXI-80-000250
        $setting = Get-AdvancedSetting -Entity $obj -Name Net.BMCNetworkEnable
        if ($setting.Value -ne 0) {
            [Void](Set-AdvancedSetting -AdvancedSetting $setting -Value 0 -Confirm:$false)
            Write-LogMessage -Message "$($esxiHost.VMHost): Configuring the ESXi host to disable virtual hardware management network interfaces. ESXI-80-000250" -Level 'UPDATE'
        } else {
            Write-LogMessage -Message "$($esxiHost.VMHost): The ESXi host is already configured to disable virtual hardware management network interfaces. ESXI-80-000250" -Level 'PASS'
        }
        ...
        # The ESXi host must forward audit records containing information to establish what type of events occurred. V-258790
        $setting = Get-AdvancedSetting -Entity $obj -Name Syslog.global.logLevel
        if ($setting.Value -ne 'info') {
            [Void](Set-AdvancedSetting -AdvancedSetting $setting -Value 'info' -Confirm:$false)
            Write-LogMessage -Message "$($esxiHost.VMHost): Configuring the ESXi host to forward audit records containing information to establish what type of events occurred. V-258790" -Level 'UPDATE'
        } else {
            Write-LogMessage -Message "$($esxiHost.VMHost): The ESXi host is already configured to forward audit records containing information to establish what type of events occurred. V-258790" -Level 'PASS'
        }

        # The ESXi must produce audit records containing information to establish what type of events occurred. ESXI-80-000015
        $setting = Get-AdvancedSetting -Entity $obj -Name Config.HostAgent.log.level
        if ($setting.Value -ne 'info') {
            [Void](Set-AdvancedSetting -AdvancedSetting $setting -Value 'info' -Confirm:$false)
            Write-LogMessage -Message "$($esxiHost.VMHost): Configuring the ESXi host to produce audit records containing information to establish what type of events occurred. ESXI-80-000015" -Level 'UPDATE'
        } else {
            Write-LogMessage -Message "$($esxiHost.VMHost): The ESXi host is already configured to produce audit records containing information to establish what type of events occurred. ESXI-80-000015" -Level 'PASS'
        }

        # The ESXi host must allocate audit record storage capacity to store at least one week's worth of audit records. V-258743
        $setting = Get-AdvancedSetting -Entity $obj -Name Syslog.global.auditRecord.storageCapacity
        if ($setting.Value -ne 100) {
            [Void](Set-AdvancedSetting -AdvancedSetting $setting -Value 100 -Confirm:$false)
            Write-LogMessage -Message "$($esxiHost.VMHost): Configuring the ESXi host to allocate audit record storage capacity to store at least one week's worth of audit records. V-258743" -Level 'UPDATE'
        } else {
            Write-LogMessage -Message "$($esxiHost.VMHost): The ESXi host is already configured to allocate audit record storage capacity to store at least one week's worth of audit records. V-258743" -Level 'PASS'
        }

        # The ESXi host must enable audit logging. ESXI-80-000232
        $setting = Get-AdvancedSetting -Entity $obj -Name Syslog.global.auditRecord.storageEnable
        if ($setting.Value -ne 'true') {
            [Void](Set-AdvancedSetting -AdvancedSetting $setting -Value 'true' -Confirm:$false)
            Write-LogMessage -Message "$($esxiHost.VMHost): Configuring the ESXi host to enable audit logging. ESXI-80-000232" -Level 'UPDATE'
        } else {
            Write-LogMessage -Message "$($esxiHost.VMHost): The ESXi host is already configured to enable audit logging. ESXI-80-000232" -Level 'PASS'
        }

        # The ESXi host DCUI.Access list must be verified. V-258751
        $setting = Get-AdvancedSetting -Entity $obj -Name DCUI.Access
        if ($setting.Value -ne 'root') {
            [Void](Set-AdvancedSetting -AdvancedSetting $setting -Value 'root' -Confirm:$false)
            Write-LogMessage -Message "$($esxiHost.VMHost): Configuring the ESXi host DCUI.Access list. V-258751" -Level 'UPDATE'
        } else {
            Write-LogMessage -Message "$($esxiHost.VMHost): The ESXi host DCUI.Access list is already configured correctly. V-258751" -Level 'PASS'
        }
